module.exports = {
    "commandLogic": async function commandLogic({message, sharder} = itemsToImport) {
        const totalBumps = await sharder.query({text: 'SELECT SUM(bumps) FROM users WHERE userid = $1', values: [message.mentions[0]?.id || message.content.split(" ")[0] || message.author.id]});
