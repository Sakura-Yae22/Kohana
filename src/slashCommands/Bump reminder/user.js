module.exports.commandLogic = async itemsToImport => {
    ...
    const mentionedUserID = interaction.data.options?.[0].value ?? interaction.member.id
    const totalBumps = await sharder.ipc.command("db", {text: 'SELECT SUM(bumps) FROM users WHERE userid = $1', values: [ mentionedUserID ]}, true);
